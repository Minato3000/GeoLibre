...
A GeoInt project captures your whole workspace in a single `.geoint.json` file: the map view, the basemap, every layer with its source and style, map preferences, plugin state, and environment variables. Everything in this section lives under the **Project** menu.

![The Project menu](https://data.geolibre.app/images/geoint-project-menu.webp)

## New

**Project → New...** starts a fresh project. GeoInt offers to save the current project first, then resets the layers, map view, controls, and plugin state to defaults.

## Open

**Project → Open From** has two sources:

- **File...** opens a `.geoint.json` file from disk (desktop app).
- **URL...** loads a public `.geoint.json` from an HTTP or HTTPS URL. This works in the browser too and adds the project to your recent list.
...
!!! note "Loading a project at startup"
    You can open a project directly by passing its URL with the `url` query parameter, for example `?url=https://share.geolibre.app/you/project.geoint.json`. See [Embedding & Sharing](embedding.md).
...
For the full schema of `.geoint.json`, including how layers, styles, and plugin state are serialized, see [Reference → Project Format](../project-format.md).
