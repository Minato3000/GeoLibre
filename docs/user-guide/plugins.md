...
Much of GeoInt's functionality ships as plugins. The **Plugins** menu activates built-in plugins, and the **Manage Plugins** dialog (under Settings) installs, updates, and removes external plugins from a curated registry.
...
![Manage Plugins dialog](https://data.geolibre.app/images/geoint-manage-plugins.webp)
...
Compatibility is checked against each entry's `minGeoIntVersion`, so incompatible plugins are flagged rather than installed.
