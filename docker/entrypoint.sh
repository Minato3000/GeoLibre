...
GEOINT_SIDECAR_TOKEN="${GEOINT_SIDECAR_TOKEN:-$(python -c 'import secrets; print(secrets.token_hex(16))')}"
export GEOINT_SIDECAR_TOKEN
...
case "$GEOINT_SIDECAR_TOKEN" in
  "" | *[!A-Za-z0-9._-]*)
    echo "GEOINT_SIDECAR_TOKEN must be non-empty and contain only [A-Za-z0-9._-]" >&2
    exit 1
    ;;
esac

# Optional AI proxy. All three values are required together so merely setting a
# model never embeds or enables ai.geolibre.app. GEOINT_AI_URL is deliberately
# restricted to the same-origin /ai route; the remote Worker URL and instance
# token remain server-side in nginx.
AI_PROXY_CONF=/etc/nginx/geoint-ai-proxy.conf
if [ -n "${GEOINT_AI_URL:-}" ] || [ -n "${GEOINT_AI_PROXY_URL:-}" ] || [ -n "${GEOINT_AI_PROXY_TOKEN:-}" ]; then
  if [ -z "${GEOINT_AI_URL:-}" ] || [ -z "${GEOINT_AI_PROXY_URL:-}" ] || [ -z "${GEOINT_AI_PROXY_TOKEN:-}" ]; then
    echo "ERROR: GEOINT_AI_URL, GEOINT_AI_PROXY_URL, and GEOINT_AI_PROXY_TOKEN must be set together." >&2
    exit 1
  fi
  case "$GEOINT_AI_URL" in
    /ai|/ai/) GEOINT_AI_URL=/ai ;;
    *)
      echo "ERROR: Docker GEOINT_AI_URL must be the same-origin path /ai." >&2
      exit 1
      ;;
  esac
  case "$GEOINT_AI_PROXY_TOKEN" in
    "" | *[!A-Za-z0-9._-]*)
      echo "ERROR: GEOINT_AI_PROXY_TOKEN must contain only [A-Za-z0-9._-]." >&2
      exit 1
      ;;
  esac
  export GEOINT_AI_URL
  export GEOINT_AI_MODEL="${GEOINT_AI_MODEL:-openai/gpt-5.5}"
  export GEOINT_AI_PROXY_URL GEOINT_AI_PROXY_TOKEN
...
upstream = os.environ["GEOINT_AI_PROXY_URL"].rstrip("/")
...
if (
    ...
    raise SystemExit(
        "ERROR: GEOINT_AI_PROXY_URL must be an HTTPS origin without credentials, path, query, or fragment."
    )
...
trusted = []
for entry in os.environ.get("GEOINT_TRUSTED_PROXIES", "").split(","):
    entry = entry.strip()
    if not entry:
        continue
    try:
        ...
        raise SystemExit(
            f"ERROR: GEOINT_TRUSTED_PROXIES entry {entry!r} is not an IP address or CIDR."
        )
real_ip = "".join(f"    set_real_ip_from {entry};\n" for entry in trusted)
if real_ip:
    real_ip += "    real_ip_header X-Forwarded-For;\n    real_ip_recursive on;\n"

token = os.environ["GEOINT_AI_PROXY_TOKEN"]
config = f"""
location /ai/ {{
{real_ip}
    proxy_pass {upstream}/;
    ...
    proxy_set_header X-GeoInt-Instance-Token "{token}";
    proxy_set_header X-GeoInt-Client-IP $remote_addr;
...
"""
open("/etc/nginx/geoint-ai-proxy.conf", "w").write(config)
'
...
deployment = {}
if os.environ.get("GEOINT_AI_URL"):
    deployment["VITE_GEOINT_AI_URL"] = os.environ["GEOINT_AI_URL"]
    deployment["VITE_GEOINT_AI_MODEL"] = os.environ["GEOINT_AI_MODEL"]
...
origins = []
for entry in os.environ.get("GEOINT_EMBED_ORIGINS", "").replace(",", " ").split():
    if entry == "*":
    ...
    if (
        ...
        raise SystemExit(
            f"ERROR: GEOINT_EMBED_ORIGINS entry {entry!r} must be an http(s) "
            "origin such as https://portal.example.com."
        )
    origins.append(f"{parsed.scheme}://{parsed.netloc}")
if origins:
    deployment["VITE_GEOINT_EMBED_ORIGINS"] = ",".join(origins)

with open("/usr/share/nginx/html/geoint-runtime-config.js", "w") as output:
    output.write("window.__GEOINT_DEPLOYMENT_ENV__ = ")
    json.dump(deployment, output, separators=(",", ":"))
    output.write(";\n")
'

if [ -n "${GEOINT_EMBED_ORIGINS:-}" ]; then
  echo "Embed postMessage API enabled for: $GEOINT_EMBED_ORIGINS"
fi
...
python -c '
import os
token = os.environ["GEOINT_SIDECAR_TOKEN"]
src = open("/etc/nginx/nginx.conf.template").read()
open("/etc/nginx/conf.d/default.conf", "w").write(
    src.replace("__GEOINT_SIDECAR_TOKEN__", token)
)
'

AUTH_CONF=/etc/nginx/geoint-auth.conf
HTPASSWD=/etc/nginx/.htpasswd

# Optional HTTP Basic Auth: when both GEOINT_AUTH_USER and
# GEOINT_AUTH_PASSWORD are set, protect the whole server (app + /sidecar
...
if [ -n "${GEOINT_AUTH_USER:-}" ] || [ -n "${GEOINT_AUTH_PASSWORD:-}" ]; then
  if [ -z "${GEOINT_AUTH_USER:-}" ] || [ -z "${GEOINT_AUTH_PASSWORD:-}" ]; then
    echo "ERROR: GEOINT_AUTH_USER and GEOINT_AUTH_PASSWORD must be set together." >&2
    exit 1
  fi
  case "$GEOINT_AUTH_USER" in
    *:*)
      echo "ERROR: GEOINT_AUTH_USER must not contain ':' (htpasswd field separator)." >&2
      exit 1
      ;;
    '#'*)
      echo "ERROR: GEOINT_AUTH_USER must not start with '#' (htpasswd treats such lines as comments)." >&2
      exit 1
      ;;
  esac
...
'
  CR=$(printf '\r')
  case "${GEOINT_AUTH_USER}${GEOINT_AUTH_PASSWORD}" in
    *"$NL"*|*"$CR"*)
      echo "ERROR: GEOINT_AUTH_USER and GEOINT_AUTH_PASSWORD must not contain newlines or carriage returns." >&2
      exit 1
      ;;
  esac
  ...
  HASH=$(printf '%s\n' "$GEOINT_AUTH_PASSWORD" | openssl passwd -6 -stdin)
  printf '%s:%s\n' "$GEOINT_AUTH_USER" "$HASH" > "$HTPASSWD"
...
EOF
  echo "HTTP Basic Auth enabled for user '$GEOINT_AUTH_USER'."
else
  printf '# Basic Auth disabled (GEOINT_AUTH_USER/GEOINT_AUTH_PASSWORD not set).\n' > "$AUTH_CONF"
  rm -f "$HTPASSWD"
fi

if [ "${GEOINT_DISABLE_SIDECAR:-0}" != "1" ]; then
